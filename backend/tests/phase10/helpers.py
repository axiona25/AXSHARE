"""Helper per test E2E Fase 10: creazione utente, upload file."""
...
def _unique_email(prefix: str = "phase10") -> str:
...
async def create_user_and_token():
    ...
    async with AsyncSessionLocal() as session:
        user = User(
            email=_unique_email("phase10"),
            display_name_encrypted="Phase10 E2E",
            role=UserRole.USER,
            is_active=True,
        )
        session.add(user)
...
async def upload_test_file(client: AsyncClient, token: str) -> str:
    ...
    original = b"test content for phase10 e2e"
    file_id_str = "phase10-e2e-id"
    ...
    metadata = FileUploadMetadata(
        name_encrypted="phase10-e2e.bin.enc",
