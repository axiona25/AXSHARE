...
/** Restituisce solo l’etichetta dell’azione (per mostrare descrizione e data/ora su due righe). */
...
export default function IMieiFilePage() {
  ...
  const { folders, revalidate: reloadFolders } = useFolders(currentFolderId)
  ...
  const handleShare = useCallback(async () => {
    if (!shareModal) return
    if (shareModal.type === 'folder') {
      showToast('La condivisione tramite link è disponibile solo per i file')
      return
    }
    ...
    try {
      ...
      if (shareModal.type === 'file') {
        fetchLinksForFile(shareModal.id)
    ...
    setShareModal(null)
  }, [shareModal, shareExpiry, shareExpiryDate, shareExpiryTime, sharePinRequired, shareRecipientUsers, resetShareModalState, showShareToast, fetchLinksForFile, refetchActivityForFile, getFileKeyBase64ForShare])
              ...
              <div className="files-title" style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                {breadcrumb.length === 0 ? (
                  <>
                    ...
                    <button type="button" className="files-title-breadcrumb-item" onClick={() => navigateBreadcrumb(-1)} data-testid="breadcrumb-root">
                      Tutti i file
                    </button>
                  </>
                ) : (
                  <>
                    ...
                    {breadcrumb.map((crumb, i) => (
                      <span key={crumb.id} className="files-title-breadcrumb-wrap">
                        ...
                        <button type="button" className="files-title-breadcrumb-item" onClick={() => navigateBreadcrumb(i)} data-testid={`breadcrumb-${crumb.id}`} style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 160 }}>
                          {crumb.name}
                        </button>
                      </span>
                    ))}
            ...
            {!showListLoading && allTableItems.length === 0 ? (
              ...
              >
                ...
                <p style={{ fontSize: 14, color: 'var(--ax-muted)', maxWidth: 320, lineHeight: 1.5 }}>
                  Questa cartella è vuota. Carica file o crea sottocartelle per iniziare.
                </p>
              </div>
            ) : (
                    ...
                    return (
                      ...
                      >
                        ...
                        <td>
                          <div className="file-name-cell">
                            <div className="file-type-icon-wrap">
                              <Image src={displayName.endsWith('.axshare') ? getAxshareFileIcon(displayName) : displayName.endsWith('.axs') ? getAxsFileIcon(displayName) : getFileIcon(displayName, (file as FileItem).is_signed)} alt={getFileLabel(displayName)} width={52} height={52} className="file-type-icon" style={{ objectFit: 'contain', flexShrink: 0 }} unoptimized />
                            </div>
              ...
              <div className="ax-share-modal-grid">
                  ...
                  <div className="ax-share-modal-section">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: 14 }}>
                        <input type="radio" name="share-permission-imf" checked={sharePermission === 'read'} onChange={() => setSharePermission('read')} />
                        <span>Lettura</span>
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: 14 }}>
                        <input type="radio" name="share-permission-imf" checked={sharePermission === 'write'} onChange={() => setSharePermission('write')} />
                ...
                <div>
                  <div className="ax-share-modal-section">
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: 14 }}>
                        <input type="radio" name="share-expiry-imf" checked={shareExpiry === 'never'} onChange={() => setShareExpiry('never')} />
                        <span>Nessuna scadenza</span>
                      </label>
                      <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', fontSize: 14 }}>
                        <input type="radio" name="share-expiry-imf" checked={shareExpiry === 'custom'} onChange={() => setShareExpiry('custom')} />
